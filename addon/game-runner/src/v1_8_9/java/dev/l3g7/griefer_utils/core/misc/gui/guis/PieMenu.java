...
import dev.l3g7.griefer_utils.core.events.render.RenderGameOverlayEvent;
import dev.l3g7.griefer_utils.core.events.render.ScaledResolutionInitEvent;
import dev.l3g7.griefer_utils.core.misc.CrosshairHider;
...
public static final ResourceLocation MISC_MENU_POINT = new ResourceLocation("griefer_utils", "icons/labymod_3/menu_point.png");
	...
	public void open(boolean animation, List<Pair<String, List<Pair<String, Runnable>>>> allPages) {
		if (open)
			return;
	...
	public void render(RenderGameOverlayEvent event) {
		...
		double midY = DrawUtils.getHeight() / 2d;
		...
		double radius = DrawUtils.getHeight() / 4d;
		...
		DrawUtils.drawCenteredString(name(), midX += offsetX, (midY += offsetY) - radius / 2.5, radius / 70);
		...
		DrawUtils.renderSkull(player().getGameProfile());
		GlStateManager.popMatrix();
		GlStateManager.pushMatrix();

		hoveredRunnable = null;
		...
		// draw crosshair
		DrawUtils.drawRect(cursorX, cursorY - 4, cursorX + 1, cursorY + 5, MAX_VALUE);
		DrawUtils.drawRect(cursorX - 4, cursorY, cursorX + 5, cursorY + 1, MAX_VALUE);

		DrawUtils.drawCenteredString(currentPage.getKey(), midX, midY + radius * 1.25, radius / 70);
		int currentPage = allPages.size() == 0 ? 0 : page + 1;
		DrawUtils.drawCenteredString(currentPage + "/" + allPages.size(), midX, midY + radius * 1.25 + radius / 7, radius / 70);
	}

	@SuppressWarnings("DuplicateExpressions")
	private void drawUnit(Pair<String, Runnable> entry, double midX, double midY, double radius, int amount, int index, double cursorX, double cursorY, double distance) {
		...
		Minecraft.getMinecraft().getTextureManager().bindTexture(MISC_MENU_POINT);
	...
	private void drawOptionTag(String displayName, double x, double y, boolean hover, double radius, int alignment) {
		...
		DrawUtils.drawRect(tagX - tagPadding, tagY - tagHeight - tagPadding, tagX + stringWidth + tagPadding, tagY + tagPadding, 0x64141414);
	...
	public void lockMouseMovementInCircle(ScaledResolutionInitEvent event) {
		...
		double radius = (double)DrawUtils.getHeight() / 4 / 3;
