...
public class GrieferInfo extends Feature {
	...
	private static final SwitchSetting profile = SwitchSetting.create()
		...
		.icon("high_res/credits/griefer_info");
	...
	private static void onGuiModify(GuiModifyItemsEvent event) {
		...
		profileLinks.putIfAbsent(name, CompletableFuture.supplyAsync(() ->
			IO.read("https://griefer.info/grieferutils/profile-link-by-name?name=" + name).tryAsJsonString().ok()));
	}
	...
	private static void onWindowClick(WindowClickEvent event) {
		...
		Option<String> profileLink = profileLinks.get(player).getNow(Option.empty());
		if (profileLink.isSet() && profileLink.get().isEmpty())
			labyBridge.notify("§e§lFehler \u26A0", "§eDieser Spieler hat kein Profil!");
		else
			labyBridge.openWebsite(profileLink.getOr("https://griefer.info/profile?keyword=" + player));
	}
