...
public abstract class PieMenu extends Gui implements Disableable {
	...
	public void open(boolean animation, List<Pair<String, List<Pair<String, Runnable>>>> allPages) {
		...
		prevCrosshairState = LabyMod.getInstance().getLabyModAPI().isCrosshairHidden();
	...
	public void close() {
		...
		LabyMod.getInstance().getLabyModAPI().setCrosshairHidden(prevCrosshairState);
		open = false;
		if (player() == null)
			return;
	...
	private void drawUnit(Pair<String, Runnable> entry, double midX, double midY, double radius, int amount, int index, double cursorX, double cursorY, double distance) {
		...
		if (finalDestX - midX > -10 && finalDestX - midX < 10) {
			LabyMod.getInstance().getDrawUtils().drawTexture(x - textureWidth / 2, y - 13, 256, 256, textureWidth, textureHeight);
			drawOptionTag(entry.getKey(), x, y, hover, radius, 0);
		} else if (finalDestX > midX) {
			LabyMod.getInstance().getDrawUtils().drawTexture(x - 13 * size, y, 256, 256, textureWidth, textureHeight);
			drawOptionTag(entry.getKey(), x, y, hover, radius, 1);
		} else {
			LabyMod.getInstance().getDrawUtils().drawTexture(x + 4, y, 256, 256, textureWidth, textureHeight);
			drawOptionTag(entry.getKey(), x, y, hover, radius, -1);
		}
