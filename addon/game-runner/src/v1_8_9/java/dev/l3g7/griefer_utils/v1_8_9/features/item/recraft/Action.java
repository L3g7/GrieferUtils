	...
	public Boolean execute(int windowId, boolean hasSucceeded) {
		if (ingredient != null) {
			...
			if (ingredientSlot == -1) {
				...
				LabyBridge.labyBridge.notifyMildError("Ein benötigtes Item ist nicht verfügbar!");
				return false;
			}
		...
		if (craftingIngredients != null) {
			for (SizedIngredient craftingIngredient : craftingIngredients) {
				if (!craftingIngredient.isAvailable()) {
					if (!hasSucceeded)
						LabyBridge.labyBridge.notifyMildError("Du hattest nicht genügend Zutaten im Inventar!");
