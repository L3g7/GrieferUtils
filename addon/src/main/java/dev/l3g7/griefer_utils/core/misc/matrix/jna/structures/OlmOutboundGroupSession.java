...
public class OlmOutboundGroupSession extends PointerType {
	...
	public static OlmOutboundGroupSession create(String encryptionKeyId) { // TODO Rotate megolm session
	...
	public static OlmOutboundGroupSession deserialize(String encryptionKeyId, String data) {
		OlmOutboundGroupSession session = allocateSession();
		...
		// Deserialize session
		size_t errorCode = LIB_OLM.olm_unpickle_outbound_group_session(session, keyBuffer, keyBuffer.size(), dataBuffer, dataBuffer.size());
		if (errorCode.equals(LIB_OLM.olm_error()))
			throw new LibOlm.OlmInvokationException("olm_unpickle_outbound_group_session", LIB_OLM.olm_outbound_group_session_last_error(session));
	...
	public String serialize() {
		...
		// Allocate memory
		size_t pickledLength = LIB_OLM.olm_pickle_outbound_group_session_length(this);
		Memory pickledBuffer = malloc(pickledLength);
		Buffer keyBuffer = malloc(encryptionKey.getBytes(UTF_8));

		// serialize session
		size_t errorCode = LIB_OLM.olm_pickle_outbound_group_session(this, keyBuffer, keyBuffer.size(), pickledBuffer, pickledLength);
		if (errorCode.equals(LIB_OLM.olm_error()))
			throw new LibOlm.OlmInvokationException("olm_pickle_outbound_group_session", LIB_OLM.olm_outbound_group_session_last_error(this));
	...
	protected void finalize() throws Throwable {
		LIB_OLM.olm_clear_outbound_group_session(this);
		super.finalize();
	}
