package dev.l3g7.griefer_utils.features.uncategorized.byteandbit.gui;
...
import dev.l3g7.griefer_utils.features.uncategorized.byteandbit.data.BABItem;
...
public class BotshopGUI extends GuiBigChest {
	...
	private void setMenuItem(BABItem entry, int slot) {
		...
		super.addItem(slot + diff, entry.getStack(), () -> {
			if (boughtItems.size() >= 7) return;
			boughtItems.add(entry);
			updatePage();
		});
	}
	...
	private boolean matchesSearch(BABItem entry, String search) {
		return entry.getStack().getDisplayName().replaceAll("§.", "").toLowerCase().contains(search.toLowerCase())
			|| ItemUtil.getLastLore(entry.getStack()).replaceAll("§.", "").toLowerCase().contains(search.toLowerCase());
	}
	...
	protected void updatePage() {
		...
		this.setGuiTitle("§2§l" + price());

		Iterator<BABItem> elemIterator = itemsDisplayed.iterator();
		boolean full = boughtItems.size() >= 7;
		for (int i = 0; i < 7; i++) {
			addItem(9 * i + 1, ItemUtil.createItem(Blocks.stained_glass_pane, 15, "§f§l<- Einkaufsliste"), () -> {});
		}
		if (!boughtItems.isEmpty()) {
			addTextureItem(28, new TextureItem("coin_pile", "§a§lKaufen(" + price() + ")", "§fBestätige mit einem Klick deinen Einkauf"), () -> {
		...
		for (int i = 0; i < 7 * 7; i++) {
			...
			BABItem item = elemIterator.next();
			setMenuItem(item, i);
		}
	}
